...
(function($) {
  ...
  function squeeze(form) {
    var fields = fieldsToValidate(form, false);
    ...
    form.submit(function(event) {
      ...
      var fields = fieldsToValidate(jQuery(event.target), true);
  ...
  function fieldsToValidate(form, hasKetchup) {
    var tags = 'input textarea select'.split(' ');
    var fields = [];

    for(var i = 0; i < tags.length; i++) {
      var selector = tags[i]+'['+options.validationAttribute+'*='+validate+']'
      if(!hasKetchup) {
        selector = selector + ":not(.hasKetchup)";
      }
