...
public class SurveyService extends AbstractController {
    ...
    public void surveysForUser(HttpServletRequest request, HttpServletResponse response)
        ...
        if (groupId > 0) {
            ...
            if(group == null){
                ...
                surveyList = surveyDAO.getActiveSurveysForUser(user, group);
            }
        } else {
    ...
    public void speciesForSurvey(
            ...
            HttpServletResponse response,
            @RequestParam(value = SURVEY_ID_PARAMETER, defaultValue = "0") int surveyPk,
    ...
    public void attributesForSurvey(
            HttpServletRequest request,
            HttpServletResponse response,
            @RequestParam(value = SURVEY_ID_PARAMETER, defaultValue = "0") int surveyId)
            throws IOException {
    ...
    public void locationsForSurvey(
        ...
        for (Location location : survey_locations) {
            Double latitude = location.getLocation().getCentroid().getX();
            Double longitude = location.getLocation().getCentroid().getY();
     ...
     * 
     * @param request contains the record form data that includes the id of the
    ...
    public void checklistForSurvey(
        ...
        if (csv_format || zip_format) {
            ...
            throw new UnsupportedOperationException(String.format("Unknown Checklist Format \"%s\"", format));
        }
    }
