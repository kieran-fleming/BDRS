...
public class CM3AuthenticationService implements UserDetailsService {
    ...
    public UserDetails loadUserByUsername(String username)
        ...
        try {
            ...
            logger.info(content + "\n");
            URL url = new URL(
                    "http://www.climatewatch.org.au/login/authenticate.aspx?"
                            + content);
            logger.info("Performing external authentication for user: "
                    + username + "\n");
            ...
            if (is != null) {
                try {
                    ...
                    logger.error("Could not lose input stream", e);
                }
            }
        }
        ...
        if (u2 == null) {
            try {
                u2 = userDAO.createUser(u.getName(), u.getFirstName(), u.getLastName(), u.getEmailAddress(), "CM3User", u.getRegistrationKey(), "ROLE_USER");
            } catch (Exception e) {
