...
OpenLayers.Layer.Tile = OpenLayers.Class(OpenLayers.Layer.HTTPRequest, {
  ...
  // Modified function: swaps source depending on string returned from TileSource()
  getTileURL: function(z, x, y, bounds) {
  		if (this.url != 'local') {
			var ext = this.type ? "." + this.type : "";
			return this.url + "/" + z + "/" + x + "/" + y + ext;
		} else {
			//Using local storage now, first checks if image is already saved
			if (localStorage.getItem(z + "/" + x + "/" + y)) {
				return localStorage.getItem(z + "/" + x + "/" + y);
			} else {
			//If image is not saved then uses the local tile cache
				var ext = this.type ? "." + this.type : "";
				return "http://192.168.2.106/TMS/" + z + "/" + x + "/" + y + ext;
			}
		}
  },
